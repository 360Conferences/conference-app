...
abstract class ScheduleItem {
  ScheduleItem(this.id, this.title, this.description, this.location, this.startTime, this.endTime);
  ...
  EventStatus computeEventStatus(DateTime time) {
    return time.isBefore(this.startTime) ? EventStatus.future
      : time.isBefore(this.endTime) ? EventStatus.present
        : EventStatus.past;
  }

  factory ScheduleItem.fromData(Map<dynamic, dynamic> data, Map<dynamic, dynamic> speakers, Map<dynamic, dynamic> favorites) {
    switch (data['type']) {
      case 'session':
        List<Speaker> items = (data['speakers'] as List<dynamic>)
          .map((id) => Speaker.fromData(speakers[id]))
          .toList();
        bool isFavorite = favorites != null && favorites.containsKey(data['id']);
        return Session(
          data['id'],
          data['title'],
          data['description'],
...
class Session extends ScheduleItem {
  Session(String id, String title, String description, String location, DateTime startTime, DateTime endTime, this.speakers, this.isFavorite)
...
class Event extends ScheduleItem {
  Event(String id, String title, String description, String location, DateTime startTime, DateTime endTime, this.address)
    : super(id, title, description, location, startTime, endTime);

  final String address;

  String get mapUrl => 'https://www.google.com/maps/search/?api=1&query=${this.location},+${this.address}'
    .replaceAll(RegExp('[\\r\\n\\s]'), '+');
}
...
class Speaker {
  ...
  Future<dynamic> get avatar {
    return FirebaseStorage.instance.ref()
      .child(kEventId).child('profiles').child(this.id)
      .getDownloadURL();
  }

  String get socialUrl => 'https://www.twitter.com/${this.social}'
    .replaceAll(RegExp('@'), '');
}
...
class Venue {
  ...
  String get mapUrl => 'https://www.google.com/maps/search/?api=1&query=${this.address}'
    .replaceAll(RegExp('[\\r\\n\\s]'), '+');
...
class SessionFeedback {
  SessionFeedback(this.overallRating, this.technicalRating, this.presentationRating, this.comment);
