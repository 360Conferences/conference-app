...
class ScheduleDetailView extends StatefulWidget {
  ScheduleDetailView({Key key, this.item, this.status, this.userSession})
      : super(key: key);
...
class _SchedulesDetailState extends State<ScheduleDetailView> {
  ...
  void _showFeedback(Session session) {
    Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              FeedbackView(session: session, userSession: widget.userSession),
      ...
      view.addAll([
        InkWell(
          child: Padding(
            padding: EdgeInsets.all(8.0),
            child: Row(
              children: <Widget>[Icon(Icons.location_on), Text(event.address)],
            ),
          ),
        ...
        slivers: <Widget>[
          ...
          SliverList(
            delegate: SliverChildListDelegate(
              (widget.item is Session)
                  ? _buildSession(widget.item)
                  : _buildEvent(widget.item),
            ),
          ),
        ],
      ),
      bottomNavigationBar: _shouldShowStatus()
          ? BottomAppBar(
              child: Row(
                ...
                children: <Widget>[
                  Padding(
                    ...
                    child: Text(
                      widget.status == EventStatus.present
                          ? 'Session in Progress'
                          : 'Session has ended',
                      style: TextStyle(
                        color: kThemeAccent,
